...
-microsoft auth login , as it's use the auth of the parent company that this instructor belong to . but for nor microsoft auth is not activated , so we will use normal email and password login 
...
- also a collection for users or instructors : will contain it's data ( id , name , email , phone) , also there will be document contain array for assigned courses , and enrrold or not , attend or absence  with it's date ofcourse
...
when instructor logged in with his account , he should see the assigned courses and if he enrolled or not , also courses or materials he finished
